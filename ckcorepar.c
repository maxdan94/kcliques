...
//Global variables (will be used to compute intersection and store k-clique degree):
unsigned *ck_p,*merge_p,*size_p;
unsigned long long *nck_p,*nck_1,*nck_2;
#pragma omp threadprivate(ck_p,merge_p,size_p,nck_p)

void allocglobal(sparse *g,unsigned kmax){
	#pragma omp parallel
	{
		ck_p=malloc(kmax*sizeof(unsigned));
		merge_p=malloc((kmax-2)*g->core*sizeof(unsigned));
		size_p=malloc((kmax-2)*sizeof(unsigned));
		nck_p=calloc(g->n,sizeof(unsigned long long));
	}
	nck_1=calloc(g->n,sizeof(unsigned long long));
	nck_2=calloc(g->n,sizeof(unsigned long long));
}
...
void recursion(unsigned kmax, unsigned k, sparse* g, unsigned* ck, unsigned* merge, unsigned* size, unsigned long long* nck){
	...
	for(i=0; i<size[k-3]; i++){
		...
		recursion(kmax, k+1, g, ck, merge, size, nck);
	}
}


//one pass over all k-cliques
void onepass(sparse *g,unsigned kmax){
	unsigned e,i;
...
// heap data structure (with value of type unsigned long long) :
...
densest* kcliquecore(unsigned kmax, unsigned long long ncktot, sparse* g){
	unsigned i,j,u,v;
	keyvalueLLU kv;
	...
	for (i=0;i<g->n-1;i++){
		...
		ds->ic[i].key=u;
		ds->ic[i].value=c;
		oneshortpass(g,heap,kmax,u);
		ncktot2=0;
		for (j=g->cd0[u];j<g->cd0[u+1];j++){
			v=g->adj0[j];
			if (heap->pt[v]!=-1){
				e--;
				updateLLU(heap,v,nck_2[v]);
				ncktot2+=nck_2[v];
				nck_2[v]=0;
			}
		}
